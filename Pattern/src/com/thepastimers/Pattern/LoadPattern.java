...
public class LoadPattern extends BukkitRunnable {
    ...
    public void run() {
        ...
        List<PatternBlock> patternBlockList = (List<PatternBlock>)database.select(PatternBlock.class,"pattern = '" + database.makeSafe(pattern) + "' order by x asc, y asc, z asc LIMIT " + done + ", " + blocksEach);
        int i = 0;
