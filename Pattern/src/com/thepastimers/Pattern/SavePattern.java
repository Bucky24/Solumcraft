...
public class SavePattern extends BukkitRunnable {
    ...
    private SavePattern(JavaPlugin plugin, Database d, Player p, String world, String pattern, int startx, int starty, int startz, int endx, int endy, int endz, int curx, int cury, int curz, int done, int total) {
        this.plugin = plugin;
        this.database = d;
        this.player = p;
        this.pattern = pattern;
    ...
    public void run() {
        ...
        if (!areWeDone) {
            BukkitTask task = new SavePattern(plugin,database,player,world,pattern,startx,starty,startz,endx,endy,endz,curx,cury,curz,done,total).runTaskLater(plugin,delay);
        } else {
